...
export const ReviewedReportsScreen = ({navigation}: any) => {
  ...
  const loadReports = async () => {
    try {
      setLoading(true);
      const data = await doctorService.getReviewedReports();
      setReports(data);
    } catch (error: any) {
      Alert.alert('Error', error.message || 'Failed to load reviewed reports');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
